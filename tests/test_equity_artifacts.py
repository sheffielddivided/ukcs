...
def _overview_changed(cwd) -> bool:
    """Mirrors the workflow's OVERVIEW_CHANGED check: `git status
    --porcelain`, not `git diff` - a bare `git diff` never reports a
    brand-new UNTRACKED file regardless of path filters (it only ever
    diffs already-tracked content), which is exactly the case a
    code-only change that adds a new derived overview artifact produces.
    docs/data/overview/meta.json's own built_at is excluded, since it
    changes on every run regardless of whether anything else changed."""
    import subprocess

    result = subprocess.run(
        ["git", "status", "--porcelain", "--", "docs/data/overview"], cwd=cwd, check=True, capture_output=True, text=True
    )
    lines = [line for line in result.stdout.splitlines() if not line.endswith("docs/data/overview/meta.json")]
    return len(lines) > 0
...
def temp_data_repo(tmp_path):
    ...
    (repo / "docs" / "data" / "overview").mkdir(parents=True)
    _git(repo, "init", "-q")
    _git(repo, "config", "user.email", "test@example.com")
    _git(repo, "config", "user.name", "Test")
    (repo / "docs" / "data" / "fields.geojson").write_text('{"a": 1}')
    (repo / "docs" / "data" / "equity" / "meta.json").write_text('{"b": 1}')
    (repo / "docs" / "data" / "overview" / "meta.json").write_text('{"built_at": "2026-01-01"}')
    (repo / "docs" / "data" / "overview" / "monthly_totals.json").write_text('{"c": 1}')
    _git(repo, "add", "-A")
...
def test_new_overview_artifact_file_triggers_substantive_diff(temp_data_repo):
    """Regression test (2026-09-10 continuation): a code-only ETL change
    that adds a brand-new derived overview artifact (e.g.
    company_groups_field_breakdown.json) must still be detected as a
    substantive change even though no upstream PPRS/equity source data
    changed at all - otherwise the new file would be generated by the
    build but never committed (the workflow's own "Commit and push" step
    is skipped whenever changed=false)."""
    (temp_data_repo / "docs" / "data" / "overview" / "company_groups_field_breakdown.json").write_text('{"d": 1}')
    # The bug this fixes: a bare `git diff` genuinely does NOT see this
    # brand-new untracked file at all, regardless of path filter.
    assert _diff_changed(temp_data_repo, "docs/data/overview") is False
    # git status --porcelain does see it.
    assert _overview_changed(temp_data_repo) is True


def test_overview_meta_only_change_does_not_trigger_diff(temp_data_repo):
    """overview/meta.json's own built_at changes on every run regardless
    of whether anything else in overview/ changed - must not, by
    itself, cause a spurious commit every week."""
    (temp_data_repo / "docs" / "data" / "overview" / "meta.json").write_text('{"built_at": "2026-01-08"}')
    assert _overview_changed(temp_data_repo) is False
