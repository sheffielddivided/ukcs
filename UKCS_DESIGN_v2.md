...
**Follow-up fix, same day**: the first live rebuild of `company_groups_field_breakdown.json`
failed - `validate_company_field_breakdown_reconciliation()`'s fixed `1e-6` tolerance was too tight
for real multi-field companies (double-rounding: each field-period and each group-period
`total_mboed` is independently rounded once at serialization, so summing several independently-
rounded field values does not, in general, exactly equal a separately-rounded group total - a real
diff up to ~0.003 for ITHACA ENERGY, well beyond `1e-6`, on entirely correct data). Fixed by
requiring a real `tolerance` argument and computing it in `etl/build.py` via
`etl/validate.py`'s `compute_serialization_tolerance()` - the same statistically-derived bound
already used for the structurally identical fields-overview-vs-monthly-totals check - instead of
an arbitrary epsilon. Also fixed `.github/workflows/build-data.yml`'s "substantive change" check,
which only ever used `git diff` (never reports a brand-new UNTRACKED file, regardless of path
filter) over `docs/data/fields.geojson`/`history`/`operators` and `docs/data/equity` - never
`docs/data/overview` at all - so a code-only change that adds a new derived overview artifact (as
this one is) would have been generated by the build and then silently discarded, uncommitted. Now
also checks `git status --porcelain -- docs/data/overview` (which does see untracked files),
excluding `overview/meta.json`'s own always-changing `built_at` the same way the top-level
`meta.json`'s is already excluded. Both fixes tested (`tests/test_overview.py`'s
`test_company_field_breakdown_reconciliation_needs_a_real_tolerance_not_exact_equality`,
`tests/test_equity_artifacts.py`'s `test_new_overview_artifact_file_triggers_substantive_diff` /
`test_overview_meta_only_change_does_not_trigger_diff`) before a manually-triggered rebuild
confirmed both fixes end-to-end and published the artifact (38 company groups) live.

### 17.11 Production overview — Top 6 + Other for company field/company breakdowns (2026-09-10) — IMPLEMENTED

Requested: "the number of fields is too large. I prefer to show only the 6 largest fields
specifically, and group the rest as 'other'. the same goes for the 'by company' view when all
companies are shown." (Reported live after 17.10 shipped - a real company, e.g. ADURA ENERGY, has
20 fields; the full "By company" view has 38 approved groups - both unreadable stacked in full.)

`docs/app/production.js`'s new `rankByLatestTotal(entries, topN)` ranks `{series}` entries by
latest-available `total_mboed` (the same defensible "rank by latest known figure" choice the
UKCS-wide By field split's own Top N already uses) and splits into the top N and the rest, shared
by both call sites - `TOP_N_DEFAULT = 6`, fixed, not a user-selectable control like the UKCS-wide
By field split's own Top N (neither of these was asked to grow one):

- **Single-company By field** (`fieldCategorySeries`): "Other fields" is defined subtractively -
  that company's own published `total_mboed` minus the sum of the displayed top 6 - reconciling
  exactly by construction (the same guarantee `etl/overview.py`'s build-breaking company&lt;-&gt;field
  check already provides at the company grain).
- **"By company" with multiple/all companies selected**: "Other companies" is a DIRECT sum of the
  excluded companies' own totals - there is no independent grand total at this grain to reconcile
  subtractively against (the stacked bars are already the whole of what's selected), so a direct
  sum is exactly self-consistent by construction on its own.
